...
const unreadCounters = document.getElementsByClassName("unread-messages-counter")
const attachmentDeleteBtn = document.getElementsByClassName("delete-attachment")
...
function updateMoveButtons(messageId, folder) {
    ...
    const messageMoveButtons = document.querySelectorAll(`.move-btn[data-message="${messageId}"]`);
...
function swapReadTogglers(messageId, isRead) {
    ...
    const messageTogglers = document.querySelectorAll(`.read-toggler[data-message="${messageId}"]`);
    if (isRead == true) {
